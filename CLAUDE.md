...
## 0. 跨機器設定與驗證 (Cross-machine setup)

這個專案可在多台機器(Windows / macOS)上 `git clone` 後使用。**當使用者問「我還需要設定什麼 / 驗證我的設定 / 在新電腦怎麼跑」之類**,你要:
1. 跑 **`node scripts/check-setup.mjs`**(從專案根目錄)——它會逐項檢查 Node、各子套件 `npm install`、原生模組能否載入、`GEMINI_API_KEY`、`kg/system-design.db`、課程素材、啟動器、設定檔有無殘留的絕對路徑,印出 ✅/⚠️/❌。
2. 讀它的輸出,**針對每個 ❌/⚠️ 給白話修正建議**;細節以 `SETUP.md` 為準(別自己編步驟)。
3. ⚠️ `.claude/settings.json`(hook 設定)Claude 預設**不會自己改**(安全保護)。若它殘留 `D:/…` 絕對路徑導致 hook 失效,**請使用者手動改成相對路徑**(見 `SETUP.md` 步驟 6),或由使用者明確授權你改。

> 路徑慣例:`.mcp.json` 與 hook 都用**相對專案根目錄**的路徑(如 `kg/main.js`),別再寫死絕對路徑,才能跨機器。
